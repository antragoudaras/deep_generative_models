...
def train_aae(epoch, model, train_loader,
    ...
    for batch_idx, (x, _) in enumerate(train_loader):
        ...
        recon_x, z = model(x)
        ae_loss, ae_logg_dict = model.get_loss_autoencoder(x, recon_x, z, lambda_)
        ae_loss.backward()
        optimizer_ae.step()
        ...
        disc_loss, disc_logg_dict = model.get_loss_discriminator(z)
        disc_loss.backward()
        optimizer_disc.step()

        #update summary writers
        logger_ae.add_values(ae_logg_dict)
        logger_disc.add_values(disc_logg_dict)
        #######################
        # END OF YOUR CODE    #
        #######################
...
def main(args):
    ...
    optimizer_ae = optim.Adam([{'params': model.encoder.parameters()}, {'params': model.decoder.parameters()}], lr=args.ae_lr, betas=(0.5, 0.999))
